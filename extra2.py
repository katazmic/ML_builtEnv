...
#   print Data_tst[0]
        if j==0:
            Ntrain[i] =(int) (np.size(Data_trn)/np.size(Data_trn[0]))#/np.size(Data_trn[0]));
        # if i==0:
        print "error %d %d is %d -"%(i,j,error[i][j]);   
        if i<(Nperc-1):
#        Data_trn, Data_rem = add_randomTrn(Data_trnI,Data_trn,sizeTrn);
#        Data_trn = get_randTest(Data_trnI,sizeTrn*(i+2))
