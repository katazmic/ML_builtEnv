...
#    Data2 = np.zeros((np.size(Data_shuff[1]),np.size(Data_shuff)))
#    sizeD = np.size(Data_shuff);  
#    for i in range(sizeD):
#        for j in range(np.size(Data_shuff[1])):
#                Data2[j][i] = (float) (Data_shuff[i][j]);
...
def evaluate_tree(DataTrn,indecisInp,indexOut):
    ...
    ms = 0;
    diff = np.ones(np.size(Out_trn));
    for i in range(np.size(Out_trn)):
        diff[i] = (Out_trn[i]-Out_predtrn[i])/(Out_trn[i])
        ms = ms+diff[i];
    
    
    rms = 100*np.sqrt(ms/np.size(Out_trn));
    
    return diff,Out_trn,Out_predtrn, rms 



def evaluate_poly(DataTrn,indecisInp,indexOut,npoly):
